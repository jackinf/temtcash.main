...
namespace TemtCash.Main.IntegrationTests.Features.Customer
{
    public class when__customer_licence_is_deleted : clear_data
    {
        ...
        public async Task then_item_should_not_be_in_database()
        {
            ...
            using (var client = ApiServerFixture.Current.Server.CreateClient())
            {
                var result = await client.HttpGet<ServiceResult<CustomerResponseViewModel>>($"{Endpoint}/{_modelId}");
