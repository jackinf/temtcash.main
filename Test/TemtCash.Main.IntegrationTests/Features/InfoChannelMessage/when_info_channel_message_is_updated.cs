...
namespace TemtCash.Main.IntegrationTests.Features.InfoChannelMessage
{
    public class when_company_licence_is_updated : clear_data
    {
        ...
        public when_company_licence_is_updated()
        {
            ...
            {
                var createdModel = new Fixture().Build<Domain.Model.InfoChannelMessage>()
        ...
        {
            // Arrange
            var updatedModel = new InfoChannelMessageCreateOrUpdateRequestViewModel();
        ...
        public void with_all_properties__then_properties_should_be_updated()
        {
            ...
            ApiServerFixture.Current.DoDatabaseOperation(context =>
            {
                var model = context.InfoChannelMessages.SingleOrDefault(x => x.Id == _modelId);
