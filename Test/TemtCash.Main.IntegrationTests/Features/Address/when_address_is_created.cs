...
{
    public class when_address_is_created
    {
        ...
        {
            // Arrange
            var viewModel = new AddressCreateOrUpdateRequestViewModel();
            ...
            {
                var address = context.Addresses.Find(result.Payload);
                Assert.Null(address);
        ...
        {
            // Arrange
            var viewModel = new Fixture().Create<AddressCreateOrUpdateRequestViewModel>();
